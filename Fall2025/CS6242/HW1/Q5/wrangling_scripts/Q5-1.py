...
def data_wrangling(filter_class: str = None) -> Tuple[List[str], List[List[Any]], List[str]]:
    ...
    with open('data/q5.csv', 'r', encoding='utf-8') as f:
        ...
        # Programmatically get unique SPECIES names for options in the dropdown menu
        dropdown_options = sorted(list(set(row[0] for row in table)))
        
        # Filter, sort, and limit the table
        # Filter the data by the class column (second column)
        if filter_class:
            table = [row for row in table if row[1] == filter_class]  # Filter by class (row[1])
