...
def dataTest(Data):
    datatest = Data()
    data = 'data/Wisconsin_breast_prognostic.csv'
    #try:
...
def PCATest(Data,PCAClassifier):
    ...
    try: 
        n_components_90 = pc.n_components_for_variance(pca, threshold=0.90)
        print("Number of components to reach >=90% variance: ", n_components_90)
    except:
        print("Failed to execute n_components_for_variance()")

    top_5_features_pc1 = pc.top_pc1_contributors(pca)
    try: 
        top_5_features_pc1 = pc.top_pc1_contributors(pca)
        print("Top 5 feature names that contribute to PC1: ", top_5_features_pc1)
    except:
        print("Failed to execute top_pc1_contributors()")
